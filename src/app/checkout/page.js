...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Mapeo exacto según DATA_CONTRACTS.md
    const orderPayload = {
      ...
      status: 'PENDING', // Regla: estado inicial obligatorio
    };
    ...
    if (res.success && res.data) {
      // Guardar temporalmente la orden confirmada para mostrarla en la pantalla de éxito
      sessionStorage.setItem('emprendelink_last_order', JSON.stringify(res.data));
      clearCart();
      router.push('/checkout/confirmacion');
    } else {
      setFormError(res.error?.message || 'Ocurrió un error al procesar tu pedido.');
    }
  };

  if (items.length === 0) {
    return (
      <main style={{ maxWidth: '600px', margin: '3rem auto', padding: '1.5rem', textAlign: 'center' }}>
  ...
  return (
    <main style={{ maxWidth: '800px', margin: '2rem auto', padding: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem' }}>
        ...
        >
          <h2 style={{ fontSize: '1.2rem', marginTop: 0, borderBottom: '1px solid #e2e8f0', paddingBottom: '0.5rem' }}>
            Resumen de compra
          </h2>
